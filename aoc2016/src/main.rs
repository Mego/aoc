    ...
    ( $day:ident ) => {
        paste! {
            ...
            fn [<test_ $day>]() {
                ...
                let day = day!($day);
                let input = runtime.block_on(fetch_input(2016, day)).trim().to_string();
                {
                    let res = $day::part1(input.clone());
                    let output = aoc2016::util::submit::submit(2016, day, 1, format!("{res}"));
                    println!("{day}::part1 {}", res);
                    assert!(output.starts_with("right"), "part1 fail: {}", output);
                }
                {
                    let res = $day::part2(input);
                    let output = aoc2016::util::submit::submit(2016, day, 2, format!("{res}"));
                    println!("{day}::part2 {}", res);
                    assert!(output.starts_with("right"), "part2 fail: {}", output);
                }
            }
        }
    };
